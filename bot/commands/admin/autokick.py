...
async def init(update, context):
    ...
    await context.bot.send_message(chat_id=log_channel ,
                                   text="🔴 <b>Kick process</b> #kick\n\nChat: {}"
                                        "\nChat_id: {}\n"
                                        "User_id: <code>{}</code>\n"
                                        "Name: {}\nUsername: @{}"
                                        ...
                                        "\n\n<b>Kick reason</b>: {}".format(update.message.chat.title,
                                                                            ...
                                                                            update.message.from_user.username, reason),
                                   parse_mode=ParseMode.HTML)
    await context.bot.delete_message(update.message.chat_id, update.message.message_id)

    # set delayed action
    async def delayed_ban(context, update=update):
        try:
            ...
            await update.message.reply_text(text="Error during autokick operation")
